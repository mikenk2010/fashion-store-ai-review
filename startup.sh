...
# This script ensures everything is ready for the lecturer
...
create_env_file() {
    if [ ! -f ".env" ]; then
        ...
        cat > .env << EOF
# Flask Configuration
SECRET_KEY=fashion-store-secret-key-2024
...
verify_application() {
    ...
    local prediction_response=$(curl -s -X POST http://localhost:6600/api/predict_review \
        ...
        -d '{"review_text":"This is a great product!"}' 2>/dev/null)
...
show_status() {
    ...
    print_info "🤖 ML Models: $(ls models/*.pkl 2>/dev/null | wc -l) models trained"
    echo
